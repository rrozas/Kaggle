...
rm_garbage = function(string){
  ...
  for (tag in  garbage){
    string = gsub(tag, "", string)
  }
  return (string)
}
train$product_description = rm_garbage(train$product_description)
test$product_description = rm_garbage(test$product_description)
